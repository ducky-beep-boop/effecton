...
> **Amendment (during Task 3):** refinements are `Check[A]` values applied with `Schema.check(*checks)`, not curried functions threaded through `pipe`. ty resolves the `pipe` form to `Unknown` for union-typed schemas (`S.Float`, `S.NullOr`, `S.Union`). `Schema.pipe` from Task 1 is removed. Wherever this plan's Task 1 and Task 3 code blocks still show `pipe` overloads or curried refinement signatures, the amendment wins:
>
> ```python
> T_contra = TypeVar("T_contra", contravariant=True)
>
> @final
> class Check(Generic[T_contra]):  # noqa: UP046 - ty infers variance only for old-style TypeVars here
...
> def greater_than(n: float) -> Check[float]: ...          # likewise the other three comparisons
> ```
...
def test_filter_guards_decode_and_encode():
    even = S.Int.check(S.filter(lambda n: n % 2 == 0, message="expected an even number"))
...
Primitives are strict and never coerce: `S.Int` rejects `True` and `1.0`. Arrays decode to tuples. Refinements attach with `schema.check(...)` and guard both directions, so encoding an invalid value fails too. `S.Array` and `S.NullOr` take a `Struct` class directly; elsewhere use `S.struct_schema(User)`.
